...
> **Statut** : ✅ **implémenté et validé en live le 2026-07-12** (branche `feat/spec12-sparkle-updates`, non poussée) — jalons §4.1–4.7 faits ; **cycle d'update complet prouvé sur cette machine** : v0.0.1 depuis `~/Applications` → « Check for Updates… » → détection v9.9.9, notes markdown rendues, download, validation EdDSA, remplacement in-place, **quarantaine levée** (0 xattr), **relaunch automatique** en 9.9.9. Toutes les hypothèses 🔬 levées le même jour (§6). **Reste avant release publique** : test persistance FDA piste A (§5), création du certificat en CI, première vraie release. Note relaunch : ne se déclenche pas depuis `/private/tmp` (LaunchServices) — sans effet en `~/Applications` ou `/Applications`.
